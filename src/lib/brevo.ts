    ...
    body: JSON.stringify({
      ...
      attributes: {
        ...
        ...(splitName(payload.name).lastName
          ...
          : {}),
        SOURCE: payload.source || "Growth Assessment",
      },
    }),
  });
...
export function reportToHtml(report: string, options: ReportEmailOptions = {}) {
  const title = options.title || "Rank It Globally Notification";
  const eyebrow = options.eyebrow || "Growth Assessment";
  const intro =
    options.intro ||
    "Here is the latest Growth Assessment booking activity.";
              ...
              <tr>
                <td style="padding:12px 30px 30px">
                  <a href="https://calendly.com/rankitglobally/free-consultation" style="background:#5552D4;color:#FFFFFF;text-decoration:none;padding:13px 18px;border-radius:6px;display:inline-block;font-weight:700;font-size:14px">
                    ${options.admin ? "Open Calendly" : "Book a Growth Assessment"}
                  </a>
                  <p style="font-size:12px;line-height:1.6;color:#8A86A3;margin:18px 0 0">Rank It Globally uses Calendly and Brevo to capture bookings, update contact records, and notify the team.</p>
                </td>
              </tr>
            </table>
          </td>
