    ...
    body: JSON.stringify({
      ...
      attributes: {
        ...
        ...(splitName(payload.name).lastName
          ...
          : {}),
        SOURCE: payload.source || "Growth Audit request",
      },
    }),
  });
...
export function leadNotificationHtml(
  ...
  const title = options.title || "New Lead";
  const eyebrow = options.eyebrow || "Rank It Globally";
  const intro = options.intro || "A visitor submitted a form on the website.";
  const metaRows = [
  ...
  return `
    <div style="margin:0;padding:0;background:#F4F3FF;font-family:Arial,Helvetica,sans-serif;color:#1A1630">
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#F4F3FF;padding:28px 12px">
        <tr>
          <td align="center">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#FFFFFF;border:1px solid #E1DFFF;border-radius:14px;overflow:hidden">
              <tr>
                <td style="background:#1A1630;padding:28px 30px;color:#FFFFFF">
                  ...
                  <h1 style="font-size:24px;line-height:1.2;margin:0 0 10px;font-weight:800">${escapeHtml(title)}</h1>
